...
const minRotationDate = rotationCurrentDate
const rotationForm = reactive<RotationDraft>({
  date: minRotationDate.value,
  time: '06:00',
  from: '北坡草场',
  to: '河谷草场',
  amount: 20,
...
function submitRotation() {
  ...
  const addedSchedule = addRotationSchedule(schedule)
  if (!addedSchedule) {
...
<template>
  <PageChrome active="profile" :refresh="false" @navigate="navigate">
    <main class="content">
      ...
      <section class="panel section"><div class="panel-head"><div class="panel-title">轮换日程</div><button class="link-btn" type="button" @click="handleAddRotation">添加</button></div><div class="list"><div v-for="item in rotationSchedules" :key="item.id" class="list-row"><span class="icon-disc">{{ item.id }}</span><span class="list-main"><strong>{{ scheduleDisplayTitle(item) }}</strong><small>{{ item.detail }}</small></span><span class="status" :class="item.tone">{{ item.status }}</span></div></div></section>
