...
function submitConsultation() { if (!selectedSymptoms.value.length && !caseText.value.trim()) { showMessage('请至少选择一项症状或填写描述'); return }; chatOpen.value = true; showMessage('问诊已提交，张医生正在接入') }
function sendMessage() { const value = chatInput.value.trim(); if (!value) { showMessage('请输入补充信息'); return }; messages.value.push({ role: 'user', text: value }); chatInput.value = ''; setTimeout(() => messages.value.push({ role: 'doctor', text: '收到，我会结合耳标数据和体温曲线继续判断，请保持设备在线。' }), 500) }
</script>

<template>
  <PageChrome active="profile" :refresh="false" @navigate="navigate">
    <main class="content">
      <div class="page-head"><div><div class="eyebrow">ONLINE VETERINARY</div><h1 class="page-title">在线问诊</h1><p class="page-description">连接驻场兽医，快速判断牲畜健康状况</p></div><span class="date-chip mono">平均 3 分钟响应</span></div>
